...
public class MainWindowController {
    ...
    @FXML
    private TableView<BarrierData> barrierTable;

    @FXML
    private TableColumn<BarrierData, Integer> addressBarColumn;

    @FXML
    private TableColumn<BarrierData, String> threadsColumn;

    @FXML
    private TableColumn<BarrierData, Integer> valueBarColumn;
    ...
    public void initialize(IStmt st){
        ...
        MyIBarrierTable<Integer, MyPair> barrierTable=new MyBarrierTable<>();
        PrgState prg1 = new sample.model.PrgState(exec1, symbolT, msg, st, table, h, barrierTable);
        ...
        addressBarColumn.setCellValueFactory(new PropertyValueFactory<BarrierData, Integer>("address"));
        threadsColumn.setCellValueFactory(new PropertyValueFactory<BarrierData, String>("threads"));
        valueBarColumn.setCellValueFactory(new PropertyValueFactory<BarrierData, Integer>("value"));
    ...
    void runOneStep(ActionEvent event) throws InterruptedException, NotExistingException, EmptyException {
        ...
        if (prgList.size()>0){
            ...
            barrierTable.setItems(getBarrierData());
    ...
    public ObservableList<BarrierData> getBarrierData() throws NotExistingException {
        MyIBarrierTable<Integer, MyPair> bar=this.ctrl.getRepo().getPrgStates().get(0).getBarrier();
        ObservableList<BarrierData> result= FXCollections.observableArrayList();
        for (Integer id:bar.getContent().keySet()){
            MyPair p=bar.get(id);
            String s=p.getThreads().toString();
            BarrierData data=new BarrierData(id, s, p.getValue());
            result.add(data);
        }
        return result;
    }
