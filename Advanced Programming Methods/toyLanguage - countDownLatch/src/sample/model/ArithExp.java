...
public class ArithExp extends Exp {  //leftexpr+rightexpr
    ...
    @Override
    public int eval(MyIDictionary<String,Integer> symbolTable, MyIHeap<Integer, Integer> hp, MyILatchTable<Integer, Integer> latchTable) throws DivideByZeroException, NotExistingException, OperatorException, EmptyException {
        int left1 = left.eval(symbolTable, hp, latchTable);
        int right1 = right.eval(symbolTable, hp, latchTable);
        if (op == '+')
            return left1 + right1;
        else if (op == '-')
            return left1 - right1;
