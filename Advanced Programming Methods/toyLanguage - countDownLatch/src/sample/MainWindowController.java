...
public class MainWindowController {
    ...
    @FXML
    private TableView<LatchTableData> latchTableView;

    @FXML
    private TableColumn<LatchTableData, Integer> idColumn;

    @FXML
    private TableColumn<LatchTableData, Integer> counterValueColumn;
    ...
    public void initialize(IStmt st){
        ...
        MyILatchTable<Integer, Integer> lt=new MyLatchTable<>();
        PrgState prg1 = new sample.model.PrgState(exec1, symbolT, msg, st, table, h, lt);
        ...
        idColumn.setCellValueFactory(new PropertyValueFactory<LatchTableData, Integer>("id"));
        counterValueColumn.setCellValueFactory(new PropertyValueFactory<LatchTableData, Integer>("counterValue"));
    ...
    void runOneStep(ActionEvent event) throws InterruptedException, NotExistingException, EmptyException {
        ...
        if (prgList.size()>0){
            ...
            latchTableView.setItems(getLatchTableData());
    ...
    public ObservableList<LatchTableData> getLatchTableData() throws EmptyException, NotExistingException {
        MyILatchTable<Integer, Integer> latcht=this.ctrl.getRepo().getPrgStates().get(0).getLatchTable();

        ObservableList<LatchTableData> result= FXCollections.observableArrayList();
        for (int n:latcht.getAllKeys()){
            LatchTableData data=new LatchTableData(n,latcht.get(n));
            result.add(data);
        }
        return result;
    }
